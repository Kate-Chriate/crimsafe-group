...
/*     document.querySelectorAll('.swiper-large').forEach(swiperEl => {
    ...
    if (statSections.length) {
        ...
        function runCount(el) {
            ...
            const finalText = decimals ? target.toFixed(decimals) : Math.round(target).toString();
        ...
        function triggerSection(section) {
            ...
            section.querySelectorAll('.count-up').forEach(runCount);
        }

        function initCountObserver() {
            const pending = Array.from(statSections).filter(s => !s.dataset.counted);
            if (!pending.length) return null;
            const margin = Math.round(window.innerHeight * 0.15);
            ...
            }, { threshold: 0.15, rootMargin: `-${margin}px 0px` });
